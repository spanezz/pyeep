...
from typing import Any, cast, override, NamedTuple

import numpy as np
import scipy.signal

from pyeep.app.client import AppEventSendEvent, ClientApp
from pyeep.models.messages.gestures import GestureEvent, GestureYes, GestureNo
...
class GyroAxisBase[T](abc.ABC):
    ...
    def add(self, timestamp: float, sample: float) -> T | None:
        """
        Add a sample for the axis.

        If the sample generated a significant event, return it.
        """
        if self.bias is None:
            ...
            return self.process_sample(timestamp, sample - self.bias)
    ...
    def add_samples(
        ...
        Return the list of events possibly generated by the samples.
        """
        res: list[T] = []
        for ts, sample in zip(timestamps, samples):
            if (val := self.add(ts, cast(float, sample))) is not None:
                res.append(val)
        return res

    @abc.abstractmethod
    def process_sample(self, timestamp: float, sample: float) -> T | None:
        """
...
class SwingGesture(NamedTuple):
    #: Start time of the gesture
    start: float
    #: Gesture duration in seconds
    duration: float
    #: Gesture span in degrees (can be negative)
    angle: float


class GyroAxisSwing(GyroAxisBase[SwingGesture]):
    """Tracks back and forth swings on an axis."""

    def __init__(
        self,
        name: str,
        min_gesture_angle: float = 3,
        min_gesture_duration: float = 0.1,
    ) -> None:
        """
        Initialize the GyroAxisSwing.

        :param name: axis name
        :param min_gesture_angle: minimum angle needed (degrees) to consider a
          gesture significant
        :param min_gesture_angle: minimum duration needed (seconds) to consider
          a gesture significant
        """
        super().__init__(name)
        self.min_gesture_angle = min_gesture_angle
        ...
        self.gesture_angle: float = 0

    @override
    def process_sample(
        self, timestamp: float, sample: float
    ) -> SwingGesture | None:
        res: SwingGesture | None = None
        sign = math.copysign(1, sample)
        if self.sign is None or self.sign != sign:
            if self.sign is not None:
                assert (
                    self.gesture_start is not None
                    and self.gesture_end is not None
                )
                if (
                    abs(angle := self.gesture_angle / 52)
                    > self.min_gesture_angle
                    and (duration := self.gesture_end - self.gesture_start)
                    > self.min_gesture_duration
                ):
                    res = SwingGesture(
                        start=self.gesture_start, duration=duration, angle=angle
                    )
            # Start a new gesture
            self.sign = sign
            self.gesture_start = timestamp
            self.gesture_angle = 0
        self.gesture_angle += sample
        self.gesture_end = timestamp
        return res


class GyroAxisLast(GyroAxisBase[None]):
...
class ModeHeadYesNo(Mode, name="yesno"):
    ...
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        # self.x_axis = GyroAxisSwing("x")
        self.y_axis = GyroAxisSwing("y", min_gesture_angle=5)
        self.z_axis = GyroAxisSwing("z", min_gesture_angle=10)
    ...
    def on_gyro(
        ...
        gestures_yes = self.y_axis.add_samples(
            timestamps,
            cast(np.ndarray[tuple[int], np.dtype[np.float64]], data[1, :]),
        )
        gestures_no = self.z_axis.add_samples(
            timestamps,
            cast(np.ndarray[tuple[int], np.dtype[np.float64]], data[2, :]),
        )

        # Generate gestures from the significant events
        events: list[GestureEvent] = []
        for g in gestures_yes:
            # Keep only top to bottom gestures
            if g.angle < 0:
                continue
            self.app.log.info(
                "Yes gesture, angle:%.2f, duration: %.2f", g.angle, g.duration
            )
            events.append(
                GestureYes(
                    ts=round(g.start * 1_000_000_000),
                    duration=g.duration,
                    speed=g.angle / g.duration,
                )
            )
        for g in gestures_no:
            self.app.log.info(
                "No gesture, angle:%.2f, duration: %.2f", g.angle, g.duration
            )
            events.append(
                GestureNo(
                    ts=round(g.start * 1_000_000_000),
        ...
        for e in events:
            self.app.main_event_queue.put_nowait(AppEventSendEvent(event=e))
